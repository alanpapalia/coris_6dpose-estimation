...
# writes the array to file for analysis
def writeArrToTestFile(arr):
    file = open("testfile.txt", "w")
    shape = ''.join(str(arr.shape))
    file.write("X/Y/Z Format of " + str(shape) + " Array\n\n")
    # (nRows, nCols, dep) = arr.shape
    (nRows, nCols) = arr.shape
    for r in range(0, nRows):
        for c in range(0, nCols):
            # file.write('|' + str(arr[r][c][0]) + ', ' + str(arr[r][c][1]) + ', ' + str(arr[r][c][2]) + '| ')
            file.write('|' + str(arr[r][c]) + '| ')
        file.write('\n')
    file.close()
